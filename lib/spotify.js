...
/**
 * Gets the "metadata" for a Spotify "radio" URI.
 *
 * @param {Array|String} uris A single URI, or an Array of URIs to base the station on
 * @param {Number} length (optional) number of tracks to get. defaults to 20.
 * @param {Function} fn callback function
 * @api public
 */

Spotify.prototype.radio = function (uris, length, fn) {
  // argument surgery
  if (!Array.isArray(uris)) {
    uris = [ uris ];
  }
  ...
  if (null == length) length = 20;

  debug('radio(%j, %j)', uris, length);

  var self = this;

  var req = { body: 'GET', uri: 'hm://radio/' };
  var data = MercuryRequest.serialize(req).toString('base64');
  var salt = Math.floor(Math.random() * 0xffff);
  var radioReq = { uris: uris, salt: salt, length: length };
  var radioData = RadioRequest.serialize(radioReq).toString('base64');

  var args = [ 0, data, radioData ];
  ...
  this.sendCommand('sp/hm_b64', args, function (err, res) {
    ...
    if (data.length >= 2) {
      // success!
      obj = self._parse(RadioTracks, new Buffer(res.result[1], 'base64'));
    } else {
