...
var radio = require('./schemas').radio
var debug = require('debug')('spotify-web:station');

var Station = radio['spotify.radio.proto.Station'];
var Feedback = radio['spotify.radio.proto.Feedback'];
...
/**
 * Send feedback on a track
 *
 * @param {String} trackUri uri of the track to provide feedback on
 * @param {String} type feedback type (either 'up' or 'down')
 * @param {Function} fn callback function
 */
Station.prototype.sendFeedback = function(trackUri, type, fn) {
  this._spotify.sendProtobufRequest({
    header: {
      method: 'POST',
      uri: 'hm://radio/stations/' + this.id + '/feedback/'
    },
    payload: {
      uri: trackUri.replace('spotify:track:', ''),
      type: type
    },
    payloadSchema: Feedback
  }, fn);
};
