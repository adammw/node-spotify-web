...
var radio = schemas.radio;
var RadioRequest = radio['spotify.radio.proto.RadioRequest'];
var RadioTracks = radio['spotify.radio.proto.Tracks'];
...
/**
 * Create a Spotify Radio station
 *
 * @param {String|Array} seeds a single Track, Album, Artist, Genre or Playlist uri, or an Array of uris to seed the station with
 * @param {Object} attrs station attributes (title, titleUri, imageUri, subtitle, subtitleUri)
 * @param {Function} fn callback function
 * @api public
 */

Spotify.prototype.createStation = function(seeds, attrs, fn) {
  // argument surgery
  if (!Array.isArray(seeds)) {
    seeds = [ seeds ];
  }

  debug('createStation(%j, %j)', seeds, attrs);

  var self = this;
  this.getTracksForStation(seeds, 20, function(err, tracks) {
    if (err) return fn(err);

    console.log('getTracksForStation', tracks)

    attrs.seeds = [attrs.titleUri];

    var onStationResponse = function(err, station) {
      if (err) return fn(err);

      station._tracks = [];
      if (Array.isArray(tracks.gids)) {
        tracks.gids.forEach(function(gid) {
          station._tracks.push(gid);
        });
      }
      ...
      if (Array.isArray(tracks.feedback)) {
        tracks.feedback.forEach(function(feedback) {
          station.feedback['spotify:track:' + feedback.uri] = feedback.type;
        });
      }
    ...
    self.sendProtobufRequest({
      header: {
        method: 'POST',
        uri: 'hm://radio/stations/'
      },
      payload: attrs,
      payloadSchema: Station,
...
/**
 * Get tracks for a Spotify Radio station
 *
 * @param {String|Array} seeds
 * @param {String|Array} lastTracks (optional)
 * @param {String} stationId (optional)
 * @param {Number} length (optional)
 * @param {Function} fn callback function
 */

Spotify.prototype.getTracksForStation = function(seeds, lastTracks, stationId, length, fn) {
  // argument surgery
  if ('number' == typeof lastTracks) {
    length = lastTracks;
    lastTracks = null;
  } else if ('number' == typeof stationId) {
    length = stationId;
    stationId = null;
  }
  if ('function' == typeof lastTracks) {
    fn = lastTracks;
    lastTracks = stationId = length = null;
  } else if ('function' == typeof stationId) {
    fn = stationId;
    stationId = length = null;
  } else if ('function' == typeof length) {
    fn = length;
    length = null;
  }
  ...
  this.sendProtobufRequest({
    ...
    payload: {
      uris: seeds,
      salt: Math.floor(1E6 * Math.random()),
      length: length,
      stationId: stationId,
      lastTracks: lastTracks
    },
    payloadSchema: RadioRequest,
    responseSchema: RadioTracks
  }, fn);
};
